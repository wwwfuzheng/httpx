...
module.exports = {
    route: function(urlData, callback){
        ...
        if(matchRule && matchRule.type === ruleType['local']) {
            ...
            callback(null, fs.existsSync(localPath) ? fs.createReadStream(localPath) : '', matchRule, localPath);
            ...
            if(_.isString(proxySetting)) {
                ...
                callback(null, request(url.format(urlObj)), matchRule, url.format(urlObj));
            }
        } else {
            var proxySetting = settings.proxyMode[settings.useProxyMode];
            if(_.isString(proxySetting)) {
